...
void ydb_query_client_free(YdbQueryClient *qc) {
  YdbExceptionGuard(ydb_fail_void, nullptr, [&]() { delete qc; })();
}
...
ydb_status_t ydb_query_NOtx_execute(YdbQueryClient *qc, const char *yql,
  ...
  return YdbExceptionGuard(
      ydb_fail_status, result_details, [&]() -> ydb_status_t {
        if (!qc || !yql) {
        ...
        ydb_status_t code =
            ydb_fill_from_status(result_details, session_result);
        if (session_result.IsSuccess()) {
          ...
          if (result.IsSuccess()) {
            for (auto &rset : result.GetResultSets()) {
              resultsets_out->sets.push_back(
                  std::make_unique<YdbResultSet>(rset));
            }
          } else {
            return code;
          }
