...
TEST(StatusMapping, MapsKnownDriverStatuses) {
  ...
  EXPECT_EQ(status_to_ydb_code(NYdb::EStatus::BAD_REQUEST),
            YDB_ERR_BAD_REQUEST);
  EXPECT_EQ(status_to_ydb_code(NYdb::EStatus::NOT_FOUND), YDB_ERR_NOT_FOUND);
  EXPECT_EQ(status_to_ydb_code(NYdb::EStatus::INTERNAL_ERROR),
            YDB_ERR_INTERNAL);
...
TEST(StatusMapping, MapsTransientStatusesToConnection) {
  ...
  EXPECT_EQ(status_to_ydb_code(NYdb::EStatus::SESSION_EXPIRED),
            YDB_ERR_CONNECTION);
...
TEST(StatusMapping, RetriableClassifierMatchesTransientStatuses) {
  EXPECT_EQ(ydb_is_status_retriable(status_to_ydb_code(NYdb::EStatus::ABORTED)),
            1);
  EXPECT_EQ(
      ydb_is_status_retriable(status_to_ydb_code(NYdb::EStatus::UNAVAILABLE)),
      1);
  EXPECT_EQ(
      ydb_is_status_retriable(status_to_ydb_code(NYdb::EStatus::SESSION_BUSY)),
      1);
  EXPECT_EQ(
      ydb_is_status_retriable(status_to_ydb_code(NYdb::EStatus::BAD_REQUEST)),
      0);
}
